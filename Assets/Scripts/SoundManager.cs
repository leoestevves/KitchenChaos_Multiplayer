...
public class SoundManager : MonoBehaviour
{
    ...
    {
        volume += .1f;
        if (volume > 1f) //Colocando um limite para o volume
        {
            volume = 0f;
        }

        PlayerPrefs.SetFloat(PLAYER_PREFS_SOUND_EFFECTS_VOLUME, volume); //Salvando a customizacao do som no menu options
        PlayerPrefs.Save();
    }
